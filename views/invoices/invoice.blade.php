    ...
    <div class="full racun">
        <div class="full racun-header bottom-margin">
            ...
            <div class="float-right half">
                <p class="full bold">{{ $invoice->club->name }}</p>
                <p class="full">{{ $invoice->club->address }}</p>
                <p class="full">ŽR: IBAN HR73 23600001101437160</p>
                <p class="full">MBS: 080341252,</p>
                <p class="full">OIB: 36974788949</p>
                <p class="full">SWIFT: ZABAHR2X - 555555</p>
            </div>
        </div>
        <div class="full half float-left racun-info">
            <div class="full">
                <div class="float-left two-thirds bold">RAČUN broj11: </div><div class="float-left third bold">{{ $invoice->invoice_number }}</div>
        ...
        <div class="half float-left half racun-client">
            ...
            <div class="float-left third">
                <p class="full">{{ $invoice->user->display_name }}</p>
        ...
        <div class="full racun-table">
            <div class="full red-row table-row bold center-text">
                ...
                <div class="float-left fifty">Porez</div>
                <div class="float-left tenth">Neto</div>
            </div>
            @foreach ($invoice->items as $item)
            <div class="full table-row bottom-border center-text stretch">
                <div class="float-left thirty">{{ $item->invoiceable->name }}</div>
                <div class="float-left tenth">{{ $item->amount }} {{ $item->currency }}</div>
                <div class="float-left fifty">{{ $item->taxes[0]['tax_amount'] }} {{ $item->currency }}</div>
                <div class="float-left tenth right-align">{{ $item->total_amount }} {{ $item->currency }}</div>
            </div>
            @endforeach
            <div class="full table-row center-text">
                <div class="third float-right">
                    <div class="float-left two-thirds bold">Osnovica: </div><div class="float-right third bold right-align">{{ $invoice->amount }} {{ $invoice->currency }}</div>
                    @foreach ($invoice->taxes as $tax)
                    <div class="float-left two-thirds bold">{{ $tax['tax_type'] }} {{ $tax['tax'] }}%: </div><div class="float-right third bold right-align">{{ $tax['tax_amount'] }} {{ $invoice->currency }}</div>
                    @endforeach
                </div>
            </div>
            <div class="full red-row table-row bold center-text">
                <div class="third float-right">
                        <div class="float-left two-thirds bold">UKUPNO: </div><div class="float-right third bold right-align">{{ $invoice->total_amount }}  {{ $invoice->currency }}</div>
                </div>
            </div>
        </div>
